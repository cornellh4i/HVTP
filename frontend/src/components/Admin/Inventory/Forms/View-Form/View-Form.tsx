...
export default function ViewForm() {
  ...
  const canPublish = formData.status === "Available";

  const handlePublish = async () => {
    if (!formData.isPublic && !canPublish) return;
    try {
      if (formData?.isPublic == false) {
        ...
        showToast(
          "Lot published",
          "Your changes have been saved and published externally.",
        );
      } else {
        await updateItem(itemId, { isPublic: false });
        setFormData((p) => ({ ...p, isPublic: false }));
        showToast(
          "Lot removed from public page",
  ...
  const handleStatusChange = (newStatus: string) => {
    if (formData.isPublic && newStatus !== "Available") {
      const confirmed = confirm(
        "Unpublish this lot? Putting this lot on hold will remove this lot from the external inventory page."
      );
      if (!confirmed) return;
      setFormData((p) => ({ ...p, status: newStatus, isPublic: false }));
      updateItem(itemId, { status: newStatus, isPublic: false }).catch(() =>
      ...
      <div className="flex items-center justify-between mb-4 md:mb-8">
        ...
        <div className="hidden md:flex gap-2">
          ...
          <button
            ...
            disabled={!formData.isPublic && !canPublish}
            className="rounded bg-[#D9D9D9] px-4 py-1.5 text-sm text-black hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-[#D9D9D9]"
          >
          ...
          <section>
            <Card className="p-6">
              ...
              <div className="flex flex-col gap-5 md:grid md:grid-cols-2 md:gap-x-6 md:gap-y-5">
                ...
                <Field label="Status">
                  <SelectField value={formData.status ?? ""} onChange={handleStatusChange} options={STATUS_OPTIONS} placeholder="Status" />
                  {!canPublish && !formData.isPublic && (
                    <p className="text-xs text-gray-500">Set to &quot;Available&quot; to publish</p>
                  )}
                </Field>
                <Field label="Type">
                  <SelectField
          ...
          <div className="md:hidden pb-4">
            <div className="flex flex-col gap-2">
              <button
                onClick={handlePublish}
                disabled={!formData.isPublic && !canPublish}
                className="w-full rounded-lg bg-[#9F9E97] px-4 py-3 text-sm text-white hover:bg-[#8a897e] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {formData.isPublic ? "Unpublish" : "Publish"}
              </button>
              <button
